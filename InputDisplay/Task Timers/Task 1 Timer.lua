...
floorRef = 0x8033B1D8 --dword
fixedCamShown = 0x80332609 --byte

effMarioObj = 0x00B61158 --dword, =0 in star-select but has a specific nonzero value otherwise

restartCount = 0

-- swiped from input direction lua
function GetCurrentAction()
	for i = 1, Actions.COUNT, 1
	do
		if Actions.ACTION[i].value == memory.readdword(action) then
...
function main()
	...
	if (mode == 0) then --check for idle
		if (GetCurrentAction() == "idle") then
	...
	if (restartCount < 2) then
		...
		if (mode == 1 or mode == 2) then --death exit
			if (GetCurrentAction() == "death exit") then
				if (mode == 1) then
					finalVI = emu.framecount() - 5
					finalFrame = emu.samplecount() - 5
					-- finalFrame_Stargrab = emu.samplecount() - 1
					-- finalVI_Stargrab = emu.framecount() - 2
				end
...
function drawDisplay()
	...
	if (mode == 1) then --Display the current time while timing is active
		if (restartCount < 2) then
			...
			else wgui.text(976,625,string.format("%.2f", getTime_IGT_Frames(currentFrame - initialFrame))) end end
	elseif (mode == 3) then --Display the final times for all formats
		wgui.setcolor("#00dcbf")
		if (getTime_IGT_Frames(finalFrame - initialFrame) <= 9.99) then
		wgui.text(995,625,string.format("%.2f", getTime_IGT_Frames(finalFrame - initialFrame)))
		else wgui.text(976,625,string.format("%.2f", getTime_IGT_Frames(finalFrame - initialFrame))) end
	end
