...
function Drawing.paint()
    ...
    Drawing.drawAnalogStick(Drawing.Screen.Width + Drawing.WIDTH_OFFSET / 3 + 33, 90)
    Memory.Refresh()
    Drawing.drawInputButtons(Drawing.Screen.Width + Drawing.WIDTH_OFFSET / 3 + 23, -210)
    Drawing.drawMiscData(Drawing.Screen.Width + 13, 270)
end
...
function Drawing.drawAnalogStick(x, y)
	...
	local x_offset = 6
	wgui.text(x + x_offset - 56, y + 85, "x: " .. Joypad.input.X)
	if Joypad.input.Y == 0 then
		wgui.text(x + x_offset, y + 85, "y: 0")
	else
		wgui.text(x + x_offset, y + 85, "y: " .. -Joypad.input.Y)
	end
end
...
function Drawing.drawInputButtons(x, y)
	...
	drawInputButton(Joypad.input.Cleft, "#FFDD00", "3", wgui.ellipse, x+38, y+457, 21, 21, 8, 7)
	drawInputButton(Joypad.input.Cright, "#FFDD00", "4", wgui.ellipse, x+77, y+457, 21, 21, 9, 7)
	drawInputButton(Joypad.input.Cup, "#FFDD00", "5", wgui.ellipse, x+57, y+438, 21, 21, 8, 8)
	drawInputButton(Joypad.input.Cdown, "#FFDD00", "6", wgui.ellipse, x+57, y+478, 21, 21, 8, 8)
	wgui.setcolor(TEXT_COLOUR)
end
