...
function main()
	...
	if (mode == 0) then --check floor reference for correct floor
		if (memory.readdword(floorRef) == 0x801A98B0 or memory.readdword(floorRef) == 0x801A98E0) then
	...
	if (mode == 1 or mode == 2) then --grand star grab xcam
		if (memory.readdword(action) == 0x00001909) then
			-- print(memory.readbyte(fixedCamShown))
			if (memory.readbyte(fixedCamShown) == 4) then --bool for xcam being shown (t/f values may change, have to find in STROOP)
...
function drawDisplay()
	...
	elseif (mode == 1) then --Display the current time while timing is active
	...
	elseif (mode == 3) then --Display the final times for all formats
		...
		if (getTime(currentVI - initialVI) <= 9.99) then
		wgui.text(995,625,string.format("%.2f", getTime(finalVI - initialVI + 2)))
		else wgui.text(976,625,string.format("%.2f", getTime(finalVI - initialVI + 2))) end
	end
